...
void loop()
{
  ...
  if (tp.isTouched) 
  {
    for (int i=0; i<tp.touches; i++)
    {
      ...
      {
      gfx->fillRoundRect(bx, by, bdx, bdy, br, DARKGREY);
      apasat = 1;
       Serial.println("h++");
      }
      }
      ...
      {
       gfx->fillRoundRect(bx, by+bdy+bds, bdx, bdy, br, DARKGREY);
       apasat = 2;
       Serial.println("h--");
      }
      }
     ...
     if ((1.66*tp.points[i].x > bx+bdx+bds) and (1.66*tp.points[i].x < bx+bdx+bds+bdx))
      ...
      if ((1.66*tp.points[i].y > by) and (1.66*tp.points[i].y < by+bdy))
      {
       gfx->fillRoundRect(bx+bdx+bds, by, bdx, bdy, br, DARKGREY);
       apasat = 3;
       Serial.println("m++");
      }
      }
