...
void loop()
{
  ...
  if (tp.isTouched) 
  {
    for (int i=0; i<tp.touches; i++)
    {
      ...
      Serial.println(' ');
 //   if ((1.66*tp.points[i].x > 550) and (1.66*tp.points[i].y > 230))
      if (1.66*tp.points[i].x > 550)
      {
      if (1.66*tp.points[i].y > 230)
      {
      apasat = 1;
      }
      }
      ...
      if (apasari%2 == 1)
      {
        Serial.println("DST = 1 ! Summer time !");
        gfx->fillRect(500, 50, 800, 25, BLACK);
        gfx->setCursor(500, 50);
        gfx->setTextColor(RED,BLACK);
        gfx->setTextSize(3 /* x scale */, 3 /* y scale */, 0 /* pixel_margin */);
      ...
      {
        Serial.println("DST = 0 ! Winter time !");
        gfx->fillRect(500, 50, 800, 25, BLACK);
        gfx->setCursor(500, 50);
        gfx->setTextColor(RED,BLACK);
